...
# XmlToExcel方案思路
## 2024/1/26更新
**当前由于OPPO温控xml文件的结构内容简化，需要修改部分代码逻辑**
  - 根据孔雀温控配置文档，直接将Excel文件的标题栏固定，而不是从xml文件中获取后再生产标题
  - 设定好标题栏后，直接读取对应的key的value，然后填进去，如果没有，则填上 / 或者空着
  - 读取完一个gear_config节点后，将其父节点的名称作为场景名称，填入到第一列
